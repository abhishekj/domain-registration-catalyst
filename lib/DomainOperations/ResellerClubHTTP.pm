...
sub createCustomer {
	...
	if ( $res->is_success ) {
		...
		#		print Dumper $req;
		return { error => 1, errormessage => $perl_scalar->{message}, };
	}
...
#	print $self->{req}->uri();
	...
	if ( $res->is_success ) {
		...
		my $perl_scalar = $coder->decode( $res->content );
		if ( ref($perl_scalar) && defined $perl_scalar->{error} ) {
...
#	print STDERR Dumper $perl_scalar;
	foreach my $domain ( keys %$perl_scalar ) {
		if ( $domain =~ /\./ && ref($perl_scalar->{$domain}) && $perl_scalar->{$domain}->{status} ) {
...
sub registerDomain {
	my $self    = shift;
	my $options = shift;
	$options->{domain} = lc $options->{domain}; 
	...
	my $is_uk ;
	my $ext = ( split( /\./, $options->{domain}, 2 ) )[1];
	$is_uk = 1 if ( $ext eq 'co.uk' || $ext eq 'org.uk' || $ext eq 'eu' || $ext eq 'nz ' || $ext eq 'ca' || $ext eq 'ru');
	...
	$self->{req}->uri( $self->{url}
		  ...
		  . '&admin-contact-id='
		  . ( $is_uk==1?-1:( $options->{admincontact} || $options->{contact} ) ) 
		  . '&tech-contact-id='
		  . ( $is_uk==1?-1:( $options->{techcontact} || $options->{contact} ) )
		  . '&billing-contact-id='
		  . ( $is_uk==1?-1:( $options->{billingcontact} || $options->{contact} ) )
		  . '&invoice-option='
...
sub modifyNameserver {
	my $self    = shift;
	my $options = shift;

	$self->{req}->uri( $self->{url}
		  . 'domains/modify-ns.json?auth-userid='
		  . $self->{username}
		  . '&auth-password='
		  . $self->{password}
		  . '&order-id='
